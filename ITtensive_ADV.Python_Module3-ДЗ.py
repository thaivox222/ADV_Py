...
#в результате в списке holod будут ссылки на страницы моделей (без домена), только обрубки

result=[]#создаем пустой список для занесения наборов значений по условию ДЗ
for hols in holod:
    url2 = url + hols.strip()#соединяем домен+обрубок=получаем готовую ссылку для парсинга. Предв-но через стрип убираем возможные пробелы в начале/конце, чтобы не было сюрпризов со ссылками
...
# в строке выше создаем из наашего csv датафрейм. Будем использовать метод добавления в таблицу SQL
